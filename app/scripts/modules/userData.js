...
const dwellingTypes = ["Own home", "Rent", "Others"];

// Function to generate a blank user object for "Add New"
function generateBlankUser() {
  // Deep clone a template or create a fresh, empty structure
  return {
    id: null, // Will be assigned upon "save"
    pin: "",
    image: "", // Or a default placeholder image URL
    biodata: {
      first_name: "",
      last_name: "",
      age: null,
...
const userDropdownActions = ["Open Data", "View", "Delete"];
function transformUserForTable(user) {
  return {
    ...
    poverty_status: user.poverty_status.dwelling_place_details.type,
    pin: user.pin,
    score: user.score,
  };
}
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      resolve({
        ...
        totalItems: MOCK_FULL_DATASET.length,
      });
    }, 500);
  });
}
...
const userTableManager = createPaginatedTableManager(
  ...
  {
    fetchDataFunction: fetchUsersDataFromAPI,
...
const detailViews = [
  {
    ...
    contentGenerator: (userData) => {
      const isPinGenerated = !!userData.pin;
      const pinInputValue = userData.pin || "";
      const buttonText = isPinGenerated ? "Generated" : "Generate PIN";
      const buttonDisabled = isPinGenerated ? "disabled" : "";
      ...
      return `
        <div class="modal-view-content">
          ...
          <div class="form-group">
            ...
            <input type="text" id="user-pin" value="${pinInputValue}" readonly>
          </div>
          <button class="button button-secondary" id="generate-pin-btn" ${buttonDisabled}>${buttonText}</button>
        </div>
      `;
    },
    isCompleteChecker: (userData) => !!userData.pin,
    dataCollector: () => ({
  ...
  <div class="modal-view-content">
    ...
    <div class="biodata grid">
      <div class="form-group">
        ...
        <input type="text" id="first-name" class="modal-input" value="${
          userData.biodata.first_name || ""
        }">
      </div>
      <div class="form-group">
        <label for="last-name">Last Name:</label>
        <input type="text" id="last-name" class="modal-input" value="${
          userData.biodata.last_name || ""
        }">
      </div>
      <div class="form-group">
        <label for="email">Email:</label>
        <input type="email" id="email" class="modal-input" value="${
          userData.biodata.email || ""
        }">
      </div>
      <div class="form-group">
        <label for="date">Date:</label>
        <input type="date" id="date" class="modal-input" value="${
          userData.biodata.date || ""
        }">
      </div>
      <div class="form-group">
        <label for="age">Age:</label>
        <input type="number" id="age" class="modal-input" value="${
          userData.biodata.age || ""
        }">
      </div>
      <div class="form-group">
        <label for="gender">Gender:</label>
        <select id="gender" class="modal-input">
          <option value="Male" ${
            userData.biodata.gender === "Male" ? "selected" : ""
          }>Male</option>
          <option value="Female" ${
            userData.biodata.gender === "Female" ? "selected" : ""
          }>Female</option>
        </select>
      </div>
      <div class="form-group">
        <label for="marital-status">Marital Status:</label>
        <select id="marital-status" class="modal-input">
          <option value="Single" ${
            userData.biodata.marital_status === "Single" ? "selected" : ""
          }>Single</option>
          <option value="Married" ${
            userData.biodata.marital_status === "Married" ? "selected" : ""
          }>Married</option>
          <option value="Widowed" ${
            userData.biodata.marital_status === "Widowed" ? "selected" : ""
          }>Widowed</option>
          <option value="Divorced" ${
            userData.biodata.marital_status === "Divorced" ? "selected" : ""
          }>Divorced</option>
          <option value="Separated" ${
            userData.biodata.marital_status === "Separated" ? "selected" : ""
          }>Separated</option>
        </select>
      </div>
      <div class="form-group">
        <label for="picture">Upload Picture:</label>
        <input type="file" id="picture" class="modal-input" accept="image/*">
      </div>
      <div class="image">
        <img src="${
          userData.biodata.image || "./assets/images/indicator-image.png"
        }" alt="" />
      </div>
      <div class="form-group">
        <label for="dwelling-place">Dwelling Place:</label>
        <select id="dwelling-place" class="modal-input">
          <option value="Own shelter" ${
            userData.biodata.dwelling_place === "Own shelter" ? "selected" : ""
          }>Own shelter</option>
          <option value="Rent (Flat)" ${
            userData.biodata.dwelling_place === "Rent (Flat)" ? "selected" : ""
          }>Rent (Flat)</option>
          <option value="Rent (Two rooms)" ${
            userData.biodata.dwelling_place === "Rent (Two rooms)"
              ? "selected"
              : ""
          }>Rent (Two rooms)</option>
          <option value="Rent (One room)" ${
            userData.biodata.dwelling_place === "Rent (One room)"
              ? "selected"
              : ""
          }>Rent (One room)</option>
          <option value="Squatting" ${
            userData.biodata.dwelling_place === "Squatting" ? "selected" : ""
          }>Squatting</option>
        </select>
      </div>
      <div class="form-group">
        <label for="economic-activities">Economic Activities:</label>
        <select id="economic-activities" class="modal-input">
          <option value="Unemployed/No Business" ${
            userData.biodata.economic_activities === "Unemployed/No Business"
              ? "selected"
              : ""
          }>Unemployed/No Business</option>
          <option value="Petty Trading" ${
            userData.biodata.economic_activities === "Petty Trading"
              ? "selected"
              : ""
          }>Petty Trading</option>
          <option value="Own a Business below 100K" ${
            userData.biodata.economic_activities === "Own a Business below 100K"
              ? "selected"
              : ""
          }>Own a Business below 100K</option>
          <option value="Part-time Employed/Own a Business above 100K" ${
            userData.biodata.economic_activities ===
            "Part-time Employed/Own a Business above 100K"
              ? "selected"
              : ""
          }>Part-time Employed/Own a Business above 100K</option>
          <option value="Fully Employed" ${
            userData.biodata.economic_activities === "Fully Employed"
              ? "selected"
              : ""
          }>Fully Employed</option>
        </select>
      </div>
      <div class="form-group">
        <label for="family-support">Family Support:</label>
        <select id="family-support" class="modal-input">
          <option value="" ${
            !userData.biodata.family_support ? "selected" : ""
          }></option>
          <option value="No Support" ${
            userData.biodata.family_support === "No Support" ? "selected" : ""
          }>No Support</option>
          <option value="Limited Support" ${
            userData.biodata.family_support === "Limited Support"
              ? "selected"
              : ""
          }>Limited Support</option>
          <option value="Moderate Support" ${
            userData.biodata.family_support === "Moderate Support"
              ? "selected"
              : ""
          }>Moderate Support</option>
          <option value="Strong Support" ${
            userData.biodata.family_support === "Strong Support"
              ? "selected"
              : ""
      ...
      <div class="form-group">
        ...
        <select id="education" class="modal-input" onchange="tertiaryName()">
          <option value="" ${
            !userData.biodata.education ? "selected" : ""
          }></option>
          <option value="No Education" ${
            userData.biodata.education === "No Education" ? "selected" : ""
          }>No Education</option>
          <option value="Half Primary" ${
            userData.biodata.education === "Half Primary" ? "selected" : ""
          }>Half Primary</option>
          <option value="Full Primary" ${
            userData.biodata.education === "Full Primary" ? "selected" : ""
          }>Full Primary</option>
          <option value="Half Secondary" ${
            userData.biodata.education === "Half Secondary" ? "selected" : ""
          }>Half Secondary</option>
          <option value="Full Secondary" ${
            userData.biodata.education === "Full Secondary" ? "selected" : ""
          }>Full Secondary</option>
          <option value="Tertiary" ${
            userData.biodata.education === "Tertiary" ? "selected" : ""
          }>Tertiary</option>
        </select>
      </div>
      <div class="form-group">
        <label for="qualification">Qualification:</label>
        <input type="text" id="qualification" class="modal-input" value="${
...
`,
    ...
    dataCollector: () => ({
      biodata: {
        ...
        dwelling_place: document.getElementById("dwelling-place")?.value || "",
        economic_activities:
          document.getElementById("economic-activities")?.value || "",
        family_support: document.getElementById("family-support")?.value || "",
    ...
    contentGenerator: (userData) => `
      <div class="modal-view-content">
        ...
        <div class="form-group">
          ...
          <select id="state" class="modal-input">
            ${nigerianStates
              .map(
                (state) =>
                  `<option value="${state}" ${
                    userData.location.state === state ? "selected" : ""
                  }>${state}</option>`
              )
              .join("")}
          </select>
  ...
  },
  {
    ...
    contentGenerator: (userData) => `
      <div class="modal-view-content">
        ...
        <select id="family-marital-status" class="modal-input">
          <option value="Single" ${
            userData.poverty_status.marital_status === "Single"
              ? "selected"
              : ""
          }>Single</option>
          <option value="Married" ${
            userData.poverty_status.marital_status === "Married"
              ? "selected"
              : ""
          }>Married</option>
          <option value="Widowed" ${
            userData.poverty_status.marital_status === "Widowed"
              ? "selected"
              : ""
          }>Widowed</option>
          <option value="Divorced" ${
            userData.poverty_status.marital_status === "Divorced"
              ? "selected"
              : ""
          }>Divorced</option>
          <option value="Separated" ${
            userData.poverty_status.marital_status === "Separated"
              ? "selected"
              : ""
          }>Separated</option>
        </select>
        ...
        <div class="dwelling-place grid">
        <div class="form-group">
          <label for="dwelling-type">Own home or rent:</label>
          <select id="dwelling-type" class="modal-input">
            ${dwellingTypes
              .map(
                (type) =>
                  `<option value="${type}" ${
                    userData.poverty_status.dwelling_place_details.type === type
                      ? "selected"
                      : ""
                  }>${type}</option>`
              )
    ...
    dataCollector: () => ({
      poverty_status: {
        business_activity: {
          ...
          business_size: document.getElementById("business-size")?.value || "",
          business_location:
            document.getElementById("business-location")?.value || "",
          daily_income:
            parseInt(document.getElementById("daily-income")?.value, 10) ||
            null,
        },
        family: {
          marital_status:
          ...
          children_under_18:
            parseInt(document.getElementById("children-under-18")?.value, 10) ||
            null,
          children_earning:
            parseInt(document.getElementById("children-earning")?.value, 10) ||
            null,
        ...
        health_status: {
          major_illness: document.getElementById("major-illness")?.value || "",
        },
      },
    }),
  },
  {
      ...
      <div class="modal-view-content">
        ...
        <div id="nin-details" class="id-details-section" style="display: ${
          ...
          <div class="form-group">
            ...
            <input type="text" id="nin-input" value="${userData.nin || ""}">
          </div>
        </div>
        ...
        <div class="form-group">
          <label class="radio-label">
            <input type="radio" name="idType" value="Driver's License" id="id-type-drivers-license" ${
              userData.id_type_selected === "Driver's License" ? "checked" : ""
            }>
            Driver's License
          </label>
        </div>
        <div id="drivers-license-details" class="id-details-section" style="display: ${
          userData.id_type_selected === "Driver's License" ? "block" : "none"
        };">
          <div class="form-group">
    ...
    dataCollector: () => {
      ...
      data.drivers_license_issue_date =
        selectedIdType === "Driver's License"
          ? document.getElementById("drivers-license-issue-date")?.value || ""
          : "";
      data.drivers_license_expiry_date =
        selectedIdType === "Driver's License"
          ? document.getElementById("drivers-license-expiry-date")?.value || ""
          : "";
...
// Refactored renderCurrentView to accept the user object and a mode
function renderModalMultiView(
  targetUser,
  initialViewIndex,
  modalTitle,
  mode // 'edit' or 'add'
) {
  let currentUserState = JSON.parse(JSON.stringify(targetUser)); // Work with a mutable copy
  let currentViewIndex = initialViewIndex;

  function updateModalContent() {
    const currentView = detailViews[currentViewIndex];
    const modalBodyContent = currentView.contentGenerator(currentUserState);

    // Calculate completion status based on the current state of collected data
    const headerTabsHTML = detailViews
      .map((view, index) => {
        const isComplete = view.isCompleteChecker(currentUserState); // Check against current state
        const statusIcon =
    ...
    const saveButtonText = mode === "add" ? "Create User" : "Save Changes";
    footerButtonsHTML += `<button class="button button-primary" id="modal-save-btn">${saveButtonText}</button>`;
    footerButtonsHTML += `<button class="button button-secondary" id="modal-close-btn">Close</button>`;
    ...
    `,
      modalTitle,
      {
        ...
        onCloseCallback: () => {
          userTableManager.refresh(); // Refresh table when modal closes
        },
      }
    );

    // Attach event listeners *after* the modal content is rendered
    ...
    if (modalBackBtn) {
      modalBackBtn.addEventListener("click", () => {
        // Collect data from the current view before moving back
        const collectedData = currentView.dataCollector();
        Object.keys(collectedData).forEach((key) => {
          if (
            ...
            currentUserState[key] = {
              ...currentUserState[key],
              ...collectedData[key],
            };
          } else {
            currentUserState[key] = collectedData[key];
          }
        });

        currentViewIndex = Math.max(0, currentViewIndex - 1);
        updateModalContent(); // Re-render with new view
      });
    }
    ...
    if (modalContinueBtn) {
      modalContinueBtn.addEventListener("click", () => {
        // Collect data from the current view before moving forward
        const collectedData = currentView.dataCollector();
        Object.keys(collectedData).forEach((key) => {
          if (
            typeof collectedData[key] === "object" &&
    ...
    if (modalSaveBtn) {
      modalSaveBtn.addEventListener("click", () => {
        // Collect data from the current view before saving
        const collectedData = currentView.dataCollector();
        Object.keys(collectedData).forEach((key) => {
          if (
            typeof collectedData[key] === "object" &&
        ...
        // Handle save logic based on mode
        if (mode === "add") {
          // Assign a new ID (in a real app, this would come from the backend)
          const newId = Math.max(...MOCK_FULL_DATASET.map((u) => u.id)) + 1;
          currentUserState.id = newId;
          MOCK_FULL_DATASET.push(currentUserState);
          alert("User created successfully!");
        } else if (mode === "edit") {
          const index = MOCK_FULL_DATASET.findIndex(
            (u) => u.id === currentUserState.id
          );
          if (index !== -1) {
            MOCK_FULL_DATASET[index] = currentUserState;
            alert("User updated successfully!");
          } else {
            console.warn("User not found for update:", currentUserState);
          }
        }
    ...
    document.querySelectorAll(".modal-view-tab").forEach((tab) => {
      tab.addEventListener("click", (e) => {
        ...
        if (index !== currentViewIndex) {
          // Collect data from the current view before switching tabs
          const collectedData = currentView.dataCollector();
          Object.keys(collectedData).forEach((key) => {
            if (
              typeof collectedData[key] === "object" &&
    ...
    // Specific field-level logic that needs re-attaching on each render
      ...
      if (generatePinBtn) {
        generatePinBtn.addEventListener("click", () => {
          ...
          if (pinInput) {
            ...
            currentUserState.pin = newPin; // Update the user data directly
            generatePinBtn.disabled = true;
            generatePinBtn.textContent = "Generated";
            // No need to re-render the whole modal just for a PIN update,
            // but if other parts of the modal depend on the PIN, then re-render
            // For now, let's keep it simple and assume PIN update is local
          }
        });
      }
    }
    ...
    if (currentView.id === "id-view") {
      document.querySelectorAll('input[name="idType"]').forEach((radio) => {
        radio.addEventListener("change", (e) => {
          ...
          // No need to re-render the whole modal just for display toggle,
          // but if other parts depend on this, call updateModalContent()
        });
      });
    }
  }
...
function handleUserTableAction(event) {
  ...
  if (!user && action === "open-data") {
    console.warn(`User with ID ${itemId} not found for open-data action.`);
    return;
  }

  switch (action) {
    case "open-data":
      renderModalMultiView(
        user,
        0,
    ...
    case "delete":
      ...
      if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener("click", () => {
          ...
          const index = MOCK_FULL_DATASET.findIndex(
            (u) => u.id === parseInt(itemId)
          );
          if (index !== -1) {
            MOCK_FULL_DATASET.splice(index, 1);
          }
...
// --- New Functionality: Add New User ---
function handleAddNewUser() {
  const blankUser = generateBlankUser();
  renderModalMultiView(blankUser, 0, "Add New User", "add");
}
...
export function initUsers() {
  ...
  const addNewButton = document.querySelector(
    ".users-content .button-secondary"
  );
  if (addNewButton) {
    addNewButton.removeEventListener("click", handleAddNewUser); // Prevent multiple listeners
    addNewButton.addEventListener("click", handleAddNewUser);
  } else {
    console.warn(
      "Add New button not found in .users-content. Make sure the HTML structure is correct."
    );
  }
}
